...
export default function MapPage() {
  ...
  useEffect(() => {
    ...
    // Reset-view control: snaps back to the destination (or SF default
    // if none is set yet). Lives next to the zoom buttons.
    const ResetControl = L.Control.extend({
      ...
      onAdd: () => {
        ...
        L.DomEvent.on(btn, 'click', (e) => {
          ...
          if (dest) {
            map.flyTo([dest.lat, dest.lon], 15, { duration: 0.6 });
          } else {
            map.flyTo([37.7749, -122.4194], 14, { duration: 0.6 });
          }
        });
        return btn;
      },
  ...
  useEffect(() => {
    ...
    // even when the map is dense with parking markers.
    const pinHtml = `
    ...
    destMarkerRef.current = L.marker([destination.lat, destination.lon], {
      ...
      .bindTooltip(
        ...
        { direction: 'top', offset: [0, -50], permanent: true, className: 'pc-dest-label' },
      )
  ...
  // ── Block markers (clustered + gradient) ──────────────────────
  ...
  // bubbles show the average occupancy of their children, so the user
  // can scan whole neighborhoods at a glance.
  useEffect(() => {
    if (!mapInstanceRef.current || !leafletLoaded) return;
    const L = window.L;
    const map = mapInstanceRef.current;

    // Clear previous layer (cluster group or plain markers).
    ...
    const cluster = L.markerClusterGroup({
      ...
      // instead of a hard cliff where every cluster shatters at once.
      maxClusterRadius: (zoom) => {
        ...
        return 1; // effectively no clustering past zoom 19, but no abrupt switch
      },
    ...
    // Identify the "best" block — lowest predicted occupancy — so we
    // can highlight it and draw the user's eye to a clear winner.
    let bestBlock = null;
    blocks.forEach(b => {
    ...
    blocks.forEach(block => {
      ...
      const marker = L.marker([block.lat, block.lon], { icon, _pct: block.predicted_occupancy_pct })
